...
must_haves:
  truths:
    ...
    - "Test messages delivered successfully to employee phones following setup instructions"
...
<task type="auto">
  ...
  **טיפים (Tips):**
  ...
  5. Verify 4 sections exist with appropriate headers
  6. Verify Hebrew text is present throughout
  7. Verify troubleshooting section has numbered lists for each error
  8. Verify code commands are in code blocks or inline code format
  </verify>
  <done>
  WHATSAPP_SETUP.md contains complete Hebrew instructions covering installation, daily use, troubleshooting (with specific error message solutions), and tips for reliable operation. Documentation has clear structure with 4 sections, Hebrew text, numbered steps, and formatted code blocks.
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  **Step 4: Bulk notification test**
  1. Create 2-3 tasks for different employees (or use your phone number multiple times)
  2. Select all tasks (if bulk send UI exists) or send individually
  3. Expected: All messages sent successfully
  4. Expected: All WhatsApp messages arrive within 10 seconds
  5. Verify each message has correct recipient and content
  ...
  **Step 7: Documentation validation**
  ...
  <resume-signal>
  Type "approved" with confirmation that all 7 steps worked, or describe which step failed and how
  </resume-signal>
</task>
...
7. Bulk notifications validated for multiple recipients
</verification>
...
- [ ] Bulk notifications sent successfully to multiple recipients
