...
<task type="auto">
  ...
  5. Confirm axios calls to /send, /send-bulk, /status endpoints with WHATSAPP_GATEWAY_URL
  6. Run: npm test (if tests exist) or manually inspect code
  </verify>
  <done>
  All WhatsApp gateway errors are categorized and translated to clear Hebrew messages indicating exactly what went wrong and what action manager should take. Service correctly wires to gateway endpoints via axios.
  </done>
</task>
