...
must_haves:
  truths:
    ...
    - "Client includes employee language when sending bulk WhatsApp messages"
  artifacts:
    ...
    - path: "client/src/pages/BulkSend.jsx"
      provides: "Include employee language in bulk send request"
      contains: "language"
  key_links:
    ...
    - from: "client/src/pages/BulkSend.jsx"
      to: "POST /send-bulk"
      via: "Include employee.language in tasksByEmployee payload"
      pattern: "language.*employee\\.language"
---
...
Output: Database schema updated with language column, API endpoints support language field, manager UI has language selector dropdown in employee form, bulk send UI includes language in request payload.
</objective>
...
<task type="auto">
  <name>Add language selector to manager UI and include in bulk send</name>
  <files>
    client/src/components/EmployeeForm.jsx
    client/src/components/EmployeesPage.jsx
    client/src/pages/BulkSend.jsx
  ...
  <action>
Add language dropdown to employee creation/editing form and ensure bulk send includes language.
...
**In BulkSend.jsx (or wherever bulk WhatsApp send is implemented):**

Ensure employee.language is included when building tasksByEmployee payload:
```javascript
tasksByEmployee[employeeId] = {
  ...
  tasks: employeeTasks,
  date: formattedDate,
  language: employee.language || 'he'  // Add this field
};
```

This ensures the /send-bulk endpoint receives employee language and can translate messages appropriately.
  </action>
  <verify>
...
6. Verify employee card shows "🇬🇧 English"
7. Edit employee and change to "Russian"
8. Verify update saves and displays "🇷🇺 Русский"

Bulk send verification:
1. Navigate to bulk send page
2. Select employees and tasks
3. Open browser DevTools Network tab
4. Click "Send via WhatsApp"
5. Inspect POST /send-bulk request body
6. Verify tasksByEmployee contains language field for each employee
...
Check Network tab to verify language field sent in POST/PUT request body.
  ...
  <done>EmployeeForm has language dropdown with 4 options (he, en, ru, ar), language included in form submission, EmployeesPage displays employee language with flag emoji, BulkSend includes employee.language in /send-bulk request payload.</done>
</task>
...
3. **UI workflow (manual):**
   ...
   - Verify employee list shows "🇷🇺 Русский"
   - Edit employee, change to Arabic
   - Verify update persists and displays "🇸🇦 العربية"
...
5. **Bulk send includes language:**
   - Use manager UI to send tasks via WhatsApp
   - Check Network tab: POST /send-bulk body includes tasksByEmployee[id].language
</verification>
...
11. BulkSend includes employee.language in /send-bulk request payload
</success_criteria>
...
- UI components updated (EmployeeForm, EmployeesPage, BulkSend)
