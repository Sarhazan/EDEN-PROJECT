...
must_haves:
  ...
  key_links:
    ...
    - from: "server/routes/whatsapp.js"
      ...
      via: "Read language from tasksByEmployee data or query from DB"
      pattern: "data\\.language|SELECT language FROM employees"
---
...
Add employee language handling with server-side fallback:
```javascript
...
Plan 02 (Task 3) updates the client bulk send UI to include employee.language in the request payload. If client doesn't send it (e.g., old UI version), the server-side fallback queries it from the database. This ensures backward compatibility while maintaining the requirement from ML-04 (client sends language).
  </action>
  <verify>
...
2. **Send bulk tasks via manager UI:**
   ...
   - Verify message shows "Hello English Worker" not "שלום English Worker"
...
4. **Test Russian employee:**
   ...
   - Verify greeting is "Привет Russian Worker"
...
6. **Translation accuracy:**
  ...
  <done>WhatsApp messages translated using i18n service, employee language read from request payload with DB fallback, greeting and headers in correct language, task data preserved, link message translated.</done>
</task>
...
3. **Russian employee:**
   ...
   - Send tasks
   - Receive: "Привет [name], Задачи на [date]..."
...
8. **Fallback to DB query:**
   - Client sends request without language field
   - Server queries language from employees table
   - Message sent in correct language
</verification>

<success_criteria>
1. whatsapp.js imports i18n service and db
2. /send-bulk endpoint extracts employee language from data
3. If language not in request, server queries from employees table
4. getFixedT locks translation context per employee
5. WhatsApp message greeting uses t('greeting', {name})
6. Task list header uses t('taskListHeader', {date, count})
7. Click-to-view message uses t('clickToView')
8. Employee receives message in configured language
9. Fallback to Hebrew if language missing or invalid
10. Multiple employees in same bulk send each get correct language
11. Task titles and descriptions not translated (preserved as-is)
12. No translation errors in server logs
</success_criteria>
...
- Language fallback logic (client payload -> DB query -> default 'he')
