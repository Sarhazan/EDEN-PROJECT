...
must_haves:
  ...
  artifacts:
    ...
    - path: "server/database/schema.js"
      provides: "Composite indexes for history query performance and location_id for filtering"
  ...
  key_links:
    ...
    - from: "server/routes/history.js"
      to: "locations table"
      via: "JOIN through systems.location_id"
      pattern: "JOIN.*systems.*location_id"
...
<task type="auto">
  <name>Task 2: Create Database Indexes and Location Filtering Schema</name>
...
**Add location_id column to systems table (required for HA-06 location filtering):**

```javascript
// Location filtering support - add location_id to systems if not exists
try {
...
const testQuery = db.prepare(`
  ...
  WHERE t.status = 'completed' AND s.location_id = 1
`).all();
console.log('Location filter test:', testQuery);
```
  </action>
  <verify>
...
- WAL mode enabled
- "Added location_id column to systems table" message appears (or skip if duplicate)
- Test location filtering query returns results with location_name
- Query /api/history with 100+ tasks - response time < 100ms
  </verify>
  <done>
Composite indexes exist (idx_tasks_history, idx_tasks_retention), WAL mode enabled, EXPLAIN QUERY PLAN confirms index usage, systems.location_id column exists, location filtering via JOIN works correctly, history queries perform well with large datasets.
  </done>
</task>
...
- Check package.json - node-cron appears in dependencies with version ^3.x
...
- Transaction safety: No partial deletions on error
  ...
  <done>
node-cron installed and appears in package.json, dataRetention service exists with cleanupOldTasks and initializeDataRetention functions, cron job scheduled for 2 AM daily, logs show job initialized, manual cleanup works correctly with 2-year retention, transaction ensures atomic deletion.
  </done>
</task>
...
5. systems.location_id column exists for location filtering
6. Location filtering via JOIN works correctly
7. Scheduled cleanup job initialized at server startup
8. Manual cleanup correctly deletes tasks >2 years old, preserves newer tasks
9. All operations use parameterized queries (no SQL injection risk)
</verification>
...
- Location filtering implementation (systems.location_id)
- Cleanup job schedule and transaction safety
- Schema changes (location_id in systems table)
</output>
