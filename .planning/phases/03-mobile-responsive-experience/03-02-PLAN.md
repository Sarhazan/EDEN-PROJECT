...
must_haves:
  truths:
    ...
    - "Manager can view timeline chart with horizontal scrolling on mobile"
  artifacts:
...
**Timeline chart horizontal scroll (MOB-GRID-07):**
- Find the timeline chart section (lines 677-731: "משימות לשבוע הקרוב")
- The chart container currently has: `flex items-end justify-between gap-2`
- Mobile adaptation needed: Add horizontal scroll capability
- Wrap the flex container in an outer div with: `overflow-x-auto -mx-4 px-4`
- Keep existing flex layout: `flex items-end justify-between gap-2 h-40 pt-4`
- Mobile view: Timeline bars scroll horizontally if needed
- Desktop view: Timeline fits naturally without scroll
- Add `min-w-[60px]` to each day bar to ensure adequate tap target and readability
...
- Timeline chart container wrapped with `overflow-x-auto -mx-4 px-4`
- Each timeline day bar has `min-w-[60px]` for readability
- Mobile layout (<1024px) shows stacked columns (flex-col)
- Desktop layout (>=1024px) shows resizable horizontal columns
- No horizontal scrolling on mobile viewports (except timeline chart)
...
- Timeline chart scrolls horizontally on mobile (MOB-GRID-07 complete)
...
2. MyDayPage: Timeline chart scrolls horizontally with adequate bar width
3. AllTasksPage: Tasks in single column
4. HistoryPage: History items in single column
5. SystemsPage/SuppliersPage/EmployeesPage/LocationsPage: Entity cards in single column
6. Modal: Full-screen takeover with compact padding
7. No horizontal scrolling on any page at 320px viewport (except timeline chart scroll container)
...
2. MyDayPage: Timeline chart fits without scroll
3. All list pages: 2-column grids
4. Modal: Centered card with rounded corners
...
- Timeline chart scrolls smoothly on mobile
</verification>
...
7. Timeline chart on MyDayPage scrolls horizontally on mobile (MOB-GRID-07 complete)
...
- No horizontal scrolling at 320px viewport width (except intentional timeline scroll)
- Stats bar adapts from 2x2 to 1x4 at md: breakpoint (768px)
- Modal transforms from full-screen to card at md: breakpoint (768px)
- Timeline chart has horizontal scroll with min-w-[60px] bars
</success_criteria>
...
- Timeline chart scroll implementation details
